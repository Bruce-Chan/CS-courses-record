...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        Node currNode = contents[index];
        Node parentNode = contents[parentIndex(index)];
        if(parentNode.priority()<=currNode.priority() ){ //base case 2
            return;
        }else {
    ...
    private void sink(int index) {
        ...
        if(lefti > size && righti > size){ //base case 1: lowest level
            return;
        }
        int minIndex = min(lefti,righti);
        Node childNode = contents[minIndex];
        if(currNode.priority()<=childNode.priority()){ //base case 2
            return ;
        } else {
            swap(index,minIndex);
            sink(minIndex);
